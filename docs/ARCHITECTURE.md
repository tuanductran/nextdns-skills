...
| Markdown AST package | `packages/nextdns-markdown` | Shared unified/remark parsing, YAML frontmatter normalization, MDAST traversal, and Valibot validation | Keep parser behavior shared; add rehype only when a real HTML output consumer exists |
...
| CLI distribution | `packages/*/dist/cli.mjs` and package `bin` metadata | Publish one executable bundle per package without a checked-in `bin/` wrapper | Keep only `index.mjs`, `cli.mjs`, and required shared chunks in `dist/`; do not add standalone command artifacts |
...
The root package uses pnpm scripts and Turbo to coordinate the three workspace packages. The shared `nextdns-markdown` package parses Markdown with `unified`, `remark-parse`, `remark-frontmatter`, and `remark-gfm`; YAML values are normalized and validated with Valibot before consumers use them. It exposes MDAST helpers for headings, code blocks, links, document nodes, and text extraction. The two CLI packages consume this shared parser rather than maintaining separate line-based frontmatter implementations. Each CLI package separates reusable modules under `src/core/`, command implementations under `src/commands/`, and the dispatcher under `src/cli.ts`. The Vite-Plus pack configuration emits only `dist/index.mjs` and `dist/cli.mjs` plus required shared chunks; the package `bin` field points directly to `dist/cli.mjs`, so no checked-in `bin/` directory is needed. The dispatcher statically imports supported commands and bundles them into the single CLI artifact. `nextdns-skills-build` compiles rules into generated context, validates structure, and exposes Valibot-backed CLI option parsers from [`src/core/cli-validation.ts`](../packages/nextdns-skills-build/src/core/cli-validation.ts). Commands validate raw arguments before reading rules or mutating files; the public build index re-exports these parsers and Markdown AST helpers for programmatic consumers. `nextdns-skills-scripts` provides focused maintenance commands and the combined `audit` aggregator. The audit runs referential-integrity, frontmatter, tags, duplicate-title, and duplicate-tag checks, then includes generated statistics in its `AuditReport`. Its public package surface exports `runAudit`, `formatAuditText`, `AuditCheck`, `AuditReport`, the Valibot schemas, and `parseAuditReport`/`parseStatsReport` from [`src/index.ts`](../packages/nextdns-scripts/src/index.ts). `runAudit` validates its assembled report at the runtime boundary before returning it. `remark-rehype` and `rehype-stringify` are intentionally not dependencies because the repository currently produces agent Markdown and structured data, not HTML. The root scripts compose these checks with formatting, type-checking, Markdown lint, link validation, duplicate-code detection, and tests.

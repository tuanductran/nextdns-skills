...
- **IDN Homograph Attacks Protection**: Block domains that impersonate other domains by misusing characters from different scripts (e.g., replacing Latin "e" with Cyrillic "е").
- **Typosquatting Protection**: Block domains registered by malicious actors targeting users who mistype website addresses (e.g., `gooogle.com` instead of `google.com`).

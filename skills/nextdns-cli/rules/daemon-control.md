...
**Impact: MEDIUM** - Controlling the background service process
...
- **Run Subcommand**: Avoid using `nextdns run` unless you are debugging and need to run the service in the foreground. Permanent installations should always use the service manager via `nextdns start`.
- **Permissions**: These commands usually require administrative privileges (`sudo` on Linux/macOS, Administrator privileges on Windows).
- **Restarting**: If you manually edit the configuration file without using `nextdns config set`, you MUST restart the service for changes to take effect.
